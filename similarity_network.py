...
st.write("Zoom in into the graph for more details (item labels will become visible). The graph was constructed using 'paraphrase-MiniLM-L6-v2' language model and *pyvis* package in python. Only largest cosine similarity edges were retained such that file size is approx. 100 kb.")
...
limits = st.text_input(
    "Enter the number of hits to be returned (if omitted, the default is 20 hits) 👇")
